...
function AdminDashboard() {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response = await axios.post('http://localhost:9092/admin/add-note', formData, {
                headers: {
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 md:p-8">
            <div className="mx-auto max-w-4xl">
                <h1 className="mb-6 text-2xl font-bold text-gray-800 md:text-3xl">Add Notebooks</h1>
                ...
                <div className="rounded-lg bg-white p-6 shadow-md">
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                        ...
                        {/* Ruling */}
                        <div className="space-y-2">
                            <label htmlFor="ruling" className="block text-sm font-medium text-gray-700">
                                Ruling
                            </label>
                            <select
                                id="ruling"
                ...
                {showPreview && (
                    <div className="mt-8 rounded-lg bg-white p-6 shadow-md">
                        <h2 className="mb-4 text-xl font-semibold text-gray-800">Notebook Preview</h2>
                        <div className="grid gap-4 md:grid-cols-2">
                                ...
                                <div className="aspect-[3/4] rounded-md bg-white p-2 shadow-sm">
                                    <div className="h-full w-full rounded border-2 border-gray-300 bg-white p-4">
                                        <div className="flex h-full flex-col items-center justify-center">
                                            {cover && <p className="font-medium text-gray-700">Cover: {cover}</p>}
                                            {size && <p className="text-gray-600">Size: {size}</p>}
                                            {paperType && <p className="text-gray-600">Paper: {paperType}</p>}
                                            {ruling && <p className="text-gray-600">Ruling: {ruling}</p>}
                                            {quantity > 0 && <p className="mt-2 text-sm text-gray-500">Quantity: {quantity}</p>}
                                            <input type="file" className="w-full mb-4 p-2 text-lg border border-gray-300 rounded-md" onChange={handleImageChange} />
                            ...
                            <div className="flex flex-col justify-center space-y-4">
                                <h3 className="text-lg font-medium text-gray-800">Notebook Details</h3>
                                <ul className="space-y-2 text-gray-600">
                                    {paperType && (
                                        <li>
                                            <span className="font-medium">Paper Type:</span> {paperType}
                                        </li>
                                    )}
                                    {size && (
                                        <li>
